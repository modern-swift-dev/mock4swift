...
public struct Parameter<Value> {
    ...
    private init(specificity: Int, matcher: @escaping (Value) -> Bool) {
        self.specificity = specificity
        self.matcher = matcher
    }

    public static var any: Self { .init(specificity: 0) { _ in true } }
    public static func value(_ value: Value, by areEqual: @escaping (Value, Value) -> Bool) -> Self {
        .init(specificity: 1) { areEqual($0, value) }
    }
    public static func matching(_ predicate: @escaping (Value) -> Bool) -> Self {
        .init(specificity: 1, matcher: predicate)
    }
    public static func capturing(_ captor: ArgumentCaptor<Value>) -> Self {
        .init(specificity: 1) { value in captor.append(value); return true }
    }
    public func matches(_ value: Value) -> Bool { matcher(value) }
}
...
public final class StaticMockRegistry: @unchecked Sendable {
    ...
    public func member<Arguments, Output>(
        ...
        let lookup = Key(owner: ObjectIdentifier(owner), key: key, types: types.map(ObjectIdentifier.init))
    ...
    public func reset(owner: Any.Type, scopes: [MockScope] = Array(MockScope.all)) {
        ...
        let resets = lock.withLock { entries.compactMap { $0.key.owner == identifier ? $0.value.reset : nil } }
        resets.forEach { $0(scopes) }
    }

    public func remove(owner: Any.Type, key: String) {
        let identifier = ObjectIdentifier(owner)
        lock.withLock { entries.keys.filter { $0.owner == identifier && $0.key == key }.forEach { entries.removeValue(forKey: $0) } }
    }
}
...
public final class GenericMockRegistry: @unchecked Sendable {
    ...
    public func member<Arguments, Output>(
        ...
        let lookup = Key(key: key, types: types.map(ObjectIdentifier.init))
        if let existing = lock.withLock({ entries[lookup] }) {
            guard let member = existing.value as? MockMember<Arguments, Output> else {
    ...
    public func reset(_ scopes: [MockScope] = Array(MockScope.all)) {
        let resets = lock.withLock { entries.values.map(\.reset) }
        resets.forEach { $0(scopes) }
    }
}
